 ...
 * Returns: nothing
 */
static void context_init (struct context *cnt, AlertPreludeCtx *prelude_ctx)
{
    /*
...
static void *motion_loop(void *arg)
{
    ...
    /***** MOTION LOOP - PREPARE FOR NEW FRAME SECTION *****/
        ...
        /***** MOTION LOOP - ACTIONS AND EVENT CONTROL SECTION *****/
	        ...
	        /* Prelude - We have a motion, let's make an alert ! */
	      if ( cnt->prelude_context )
 ...
 * Returns: nothing
 */
static void cntlist_create(int argc, char *argv[], AlertPreludeCtx *prelude_ctx)
{
    ...
    context_init(cnt_list[0], prelude_ctx);
 ...
 * Returns: nothing
 */
static void motion_startup(int daemonize, int argc, char *argv[], AlertPreludeCtx *prelude_ctx)
{
    /* Initialize our global mutex */
    pthread_mutex_init(&global_lock, NULL);

    /*
     * Init Prelude library
     */
    if ( !(prelude_ctx = AlertPreludeInitCtx()) )
      motion_log(LOG_INFO, 0, "Failed to init Prelude");
    else
      motion_log(LOG_INFO, 0, "Init Prelude OK :)");

    /* Create the list of context structures and load the
     * configuration.
     */
    cntlist_create(argc, argv, prelude_ctx);
...
int main (int argc, char **argv)
{
    ...
    pthread_t thread_id;
    AlertPreludeCtx *prelude_ctx = NULL;
    ...
    motion_startup(1, argc, argv, prelude_ctx);
...
#endif
            motion_startup(0, argc, argv, prelude_ctx); /* 0 = skip daemon init */
        }
    ...
    motion_shutdown();
    prelude_ctx->clean_fct(prelude_ctx);
